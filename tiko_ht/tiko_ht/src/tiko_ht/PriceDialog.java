...
public class PriceDialog extends Dialog {
	...
	private void createContents() {
		...
		List itemList = new List(shell,
				SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL);
		itemList.setBounds(75, 185, 250, 150);
		// Check that there are used items.
		if (item_list.size() > 0) {

			// If the item_list's array length is 6, then it is a contract.
			// Hourly
			// job array size is 7.
			...
			// Go through every item and add their name, amount, unit and price
			// to
			// the list.
			for (String[] item : item_list) {
				double item_price = 0;
				// If the job is a contract, add with a different structured
				// array.
				if (contract) {
			...
			itemList.add("----------------------------------------------");
			itemList.add("Tarvikkeiden hinta: "
					+ Math.round(item_sum * 100.0) / 100.0 + " euroa");
			itemList.add(
					"Tarvikkeiden veroton hinta: " + (taxless_item_sum) + " euroa");
		} else {
			// No items added, so price is 0.
			itemList.add("----------------------------------------------");
			itemList.add("Tarvikkeiden hinta: 0 euroa.");
			itemList.add("Tarvikkeiden veroton hinta: 0 euroa.");
		}
		

		Label lblKokoHinta = new Label(shell, SWT.NONE);
		lblKokoHinta.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_TITLE_BACKGROUND_GRADIENT));
		lblKokoHinta
				.setFont(SWTResourceManager.getFont("System", 16, SWT.NORMAL));
		lblKokoHinta.setBounds(137, 341, 90, 15);
		lblKokoHinta.setText("Koko hinta");

		Label sumLabel = new Label(shell, SWT.NONE);
		sumLabel.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_TITLE_BACKGROUND_GRADIENT));
		sumLabel.setFont(
		...
		taxed_label.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_TITLE_BACKGROUND_GRADIENT));
		taxed_label.setFont(
		...
		taxless_label.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_TITLE_BACKGROUND_GRADIENT));
		...
		taxless_sumLabel.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_TITLE_BACKGROUND_GRADIENT));
		taxless_sumLabel
